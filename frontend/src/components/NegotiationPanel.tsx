    ...
    <div style={{ border: `1px solid ${S.rule}`, background: COLORS.white, marginBottom: "1.5rem" }}>
      ...
      >
        ...
        <input
          ...
          style={{ width: "1rem", height: "1rem", cursor: "pointer", accentColor: S.plum }}
        />
      </label>

      {/* Consent copy — always visible before opt-in */}
      {!opted && (
        <div style={{ padding: "0 1.25rem 1rem", borderTop: `1px solid ${S.rule}` }}>
          <p style={{
...
function QuoteAnalysisRow({
  ...
  return (
    ...
    >
      <p style={{ fontFamily: S.mono, fontSize: "0.6rem", letterSpacing: "0.08em", textTransform: "uppercase", color: S.inkLight, marginBottom: "0.5rem" }}>
      ...
      {!entry || entry === "loading" ? (
        ...
        <p style={{ fontFamily: S.mono, fontSize: "0.6rem", color: "#C94C2E" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
      ...
      <div style={{
        ...
        borderTop: `1px solid ${S.rule}`, borderLeft: `1px solid ${S.rule}`,
        fontSize: "0.75rem",
      }}>
        {[
          ...
          <div key={c.label} style={{ padding: "0.5rem 0.75rem", borderRight: `1px solid ${S.rule}`, borderBottom: `1px solid ${S.rule}`, background: COLORS.white }}>
            <div style={{ fontFamily: S.mono, fontSize: "0.5rem", letterSpacing: "0.1em", textTransform: "uppercase", color: S.inkLight, marginBottom: "0.2rem" }}>{c.label}</div>
            <div style={{ fontFamily: S.serif, fontWeight: 700, fontSize: "0.875rem", lineHeight: 1 }}>{c.value}</div>
          </div>
        ))}
      </div>

      {/* Rationale */}
      <p style={{ fontFamily: S.mono, fontSize: "0.6rem", letterSpacing: "0.03em", color: S.inkLight, lineHeight: 1.6 }}>
        {rationale}
      </p>
      ...
      {suggestedCounterCents !== undefined && (
        <div style={{
          ...
          <p style={{ fontFamily: S.mono, fontSize: "0.6rem", letterSpacing: "0.08em", textTransform: "uppercase", color: "#C94C2E", marginBottom: "0.2rem" }}>
            Suggested counter offer
          </p>
          <p style={{ fontFamily: S.serif, fontWeight: 700, fontSize: "1.25rem", lineHeight: 1, color: "#C94C2E" }}>
