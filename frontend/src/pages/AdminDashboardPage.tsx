...
function runwayColor(days: number | null): string {
  if (days === null) return UI.inkLight;
  if (days < RUNWAY_CRIT_DAYS)  return UI.rust;
...
function statusDot(status: string): { color: string; label: string } {
  switch (status) {
    ...
    default:         return { color: UI.inkLight, label: "Unknown"  };
  }
}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      ...
      {atRisk.length > 0 && (
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.875rem 1.25rem", border: `1px solid #d97706`, background: "#fffbeb" }}>
          ...
          <span style={{ fontFamily: UI.mono, fontSize: "0.65rem", color: "#92400e" }}>
      ...
      <div style={{ border: `1px solid ${UI.rule}`, background: COLORS.white }}>
        <div style={{ padding: "0.875rem 1.25rem", borderBottom: `1px solid ${UI.rule}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            ...
            {levels && (
              <span style={{ marginLeft: "0.75rem", fontFamily: UI.mono, fontSize: "0.55rem", color: UI.inkLight }}>
        ...
        {levels ? (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: "0", borderTop: `1px solid ${UI.rule}` }}>
            {levels
              .slice()
              .sort((a, b) => {
                ...
                return (
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: "0.375rem", marginBottom: "0.25rem" }}>
                      ...
                      <span style={{ fontFamily: UI.mono, fontSize: "0.65rem", fontWeight: 600, color: UI.ink }}>{l.name}</span>
                    </div>
                    <div style={{ fontFamily: UI.mono, fontSize: "0.6rem", color: dot.color, fontWeight: 600 }}>{dot.label}</div>
                    <div style={{ fontFamily: UI.mono, fontSize: "0.55rem", color: UI.inkLight, marginTop: "0.125rem" }}>
          ...
          <div style={{ padding: "1.5rem", textAlign: "center", fontFamily: UI.mono, fontSize: "0.6rem", color: UI.inkLight }}>
            {polling ? "Polling cycle levels…" : "No data — click Poll Now"}
          </div>
        )}

        <div style={{ padding: "0.5rem 1rem", borderTop: `1px solid ${UI.rule}` }}>
          <span style={{ fontFamily: UI.mono, fontSize: "0.5rem", color: UI.inkLight }}>
      ...
      <div style={{ border: `1px solid ${UI.rule}`, background: COLORS.white }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.75rem", fontFamily: UI.mono }}>
          <thead>
            <tr style={{ borderBottom: `1px solid ${UI.rule}` }}>
              {["Canister", "Balance", "Daily Burn", "Runway", "Memory", "Error Rate"].map((h) => (
                <th key={h} style={{ padding: "0.625rem 1rem", textAlign: "left", fontWeight: 400, fontSize: "0.6rem", letterSpacing: "0.1em", textTransform: "uppercase", color: UI.inkLight }}>
                  {h}
                </th>
              ))}
            </tr>
            ...
            {metrics
              ...
              .map((m) => {
                ...
                return (
                  <tr
                    key={m.canisterId}
                    style={{ borderBottom: `1px solid ${UI.rule}`, background: rowAlert ? "#fff1f2" : "transparent" }}
                  >
                    <td style={{ padding: "0.625rem 1rem", fontSize: "0.65rem", color: UI.ink }}>
                      <code style={{ fontSize: "0.6rem" }}>{m.canisterId.slice(0, 18)}…</code>
                    </td>
                    <td style={{ padding: "0.625rem 1rem", color: UI.ink }}>
                    ...
                    <td style={{ padding: "0.625rem 1rem", color: UI.inkLight }}>{memPct}</td>
                    <td style={{ padding: "0.625rem 1rem", color: m.errorCount > 0 ? UI.rust : UI.inkLight }}>{errRate}</td>
                  </tr>
                );
              })}
          </tbody>
    ...
    <div style={{ border: `1px solid ${UI.rule}`, background: COLORS.white, padding: "1.5rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
      <div>
        ...
        <p style={{ fontFamily: UI.mono, fontSize: "0.6rem", letterSpacing: "0.06em", color: UI.inkLight }}>
          ID: {String(property.id)} · Owner: <code style={{ fontSize: "0.7rem", background: UI.paper, padding: "0.1rem 0.3rem" }}>{property.owner}</code>
        </p>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
          <span style={{ fontFamily: UI.mono, fontSize: "0.6rem", letterSpacing: "0.1em", textTransform: "uppercase", color: UI.inkLight }}>Approve as:</span>
          <select
            value={level}
...
function TierManager() {
  ...
  return (
    <div style={{ border: `1px solid ${UI.rule}`, background: COLORS.white, padding: "1.5rem" }}>
      ...
      <div style={{ marginTop: "1.5rem", border: `1px solid ${UI.rule}`, padding: "1rem" }}>
      ...
      <div style={{ maxWidth: "60rem", margin: "0 auto", padding: "2rem 1.5rem" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "2rem", flexWrap: "wrap", gap: "1rem" }}>
          <div>
            ...
            <p style={{ fontFamily: UI.mono, fontSize: "0.6rem", letterSpacing: "0.06em", color: UI.inkLight }}>
              Principal: <code style={{ background: UI.paper, padding: "0.1rem 0.4rem" }}>{principal}</code>
            </p>
          </div>
        </div>
          ...
          {([
            ...
            <button
              ...
              style={{
                ...
                color: tab === t.id ? UI.rust : UI.inkLight,
                border: "none", borderBottom: tab === t.id ? `2px solid ${UI.rust}` : "2px solid transparent",
        ...
        {tab === "verifications" && (
          <div>
            ...
            {loadingPending ? (
              ...
              <div style={{ border: `1px dashed ${UI.rule}`, padding: "3rem", textAlign: "center" }}>
                <CheckCircle size={32} color={UI.rule} style={{ margin: "0 auto 0.75rem" }} />
                <p style={{ fontFamily: UI.mono, fontSize: "0.65rem", letterSpacing: "0.06em", color: UI.inkLight }}>No pending verifications. All caught up!</p>
              </div>
            ) : (
